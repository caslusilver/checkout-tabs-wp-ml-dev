...
function checkout_tabs_wp_ml_render_admin_page(): void {
	...
	$splash_enabled = ((int) get_option('checkout_tabs_wp_ml_splash_enabled', 0) === 1);
	$splash_bg = sanitize_hex_color((string) get_option('checkout_tabs_wp_ml_splash_bg', '#ffdb15')) ?: '#ffdb15';
	$splash_image_url = esc_url((string) get_option('checkout_tabs_wp_ml_splash_image_url', ''));
	$splash_duration_ms = absint(get_option('checkout_tabs_wp_ml_splash_duration_ms', 1200));
	$splash_text_enabled = ((int) get_option('checkout_tabs_wp_ml_splash_text_enabled', 0) === 1);
	$splash_text = sanitize_text_field((string) get_option('checkout_tabs_wp_ml_splash_text', ''));
	$splash_text_color = sanitize_hex_color((string) get_option('checkout_tabs_wp_ml_splash_text_color', '#111111')) ?: '#111111';
	$splash_text_font = sanitize_text_field((string) get_option('checkout_tabs_wp_ml_splash_text_font', 'Arial')) ?: 'Arial';
	$splash_text_gap_px = max(10, absint(get_option('checkout_tabs_wp_ml_splash_text_gap_px', 12)));
	$splash_text_typing = ((int) get_option('checkout_tabs_wp_ml_splash_text_typing', 0) === 1);
	...
	echo '<div class="ctwpml-admin-tab-panel" data-tab="integracoes" style="' . esc_attr($style_integracoes) . '">';
		...
		echo '</tr>';
		echo '<tr>';
		echo '<th scope="row"><label for="checkout_tabs_wp_ml_privacy_policy_url">URL da Política de Privacidade</label></th>';
		echo '<td>';
		echo '<input type="url" class="regular-text" id="checkout_tabs_wp_ml_privacy_policy_url" name="checkout_tabs_wp_ml_privacy_policy_url" value="' . esc_attr($privacy_policy_url) . '" placeholder="https://..." />';
		echo '<p class="description">Se preenchido, o texto “política de privacidade” no Review vira link clicável (sem hardcode).</p>';
		echo '</td>';
		echo '</tr>';

		echo '<tr><th colspan="2"><h3 style="margin:18px 0 8px;">Header do Modal (ML)</h3></th></tr>';
		echo '<tr>';
		echo '<th scope="row">Cores do Header</th>';
		echo '<td>';
		echo '<div style="display:flex; gap: 16px; flex-wrap: wrap; align-items: center;">';
		echo '<label style="display:flex; gap:8px; align-items:center;">Fundo <input type="color" name="checkout_tabs_wp_ml_ml_header_bg" value="' . esc_attr($ml_header_bg) . '" /></label>';
		echo '<label style="display:flex; gap:8px; align-items:center;">Título <input type="color" name="checkout_tabs_wp_ml_ml_header_title_color" value="' . esc_attr($ml_header_title_color) . '" /></label>';
		echo '<label style="display:flex; gap:8px; align-items:center;">Ícone <input type="color" name="checkout_tabs_wp_ml_ml_header_icon_color" value="' . esc_attr($ml_header_icon_color) . '" /></label>';
		echo '</div>';
		echo '<p class="description">Personaliza apenas cores do header do modal (fundo/título/ícone voltar).</p>';
		echo '</td>';
		echo '</tr>';

		echo '<tr><th colspan="2"><h3 style="margin:18px 0 8px;">Splash Screen (Identidade Visual)</h3></th></tr>';
		echo '<tr>';
		echo '<th scope="row">Ativar Splash</th>';
		echo '<td><label><input type="checkbox" name="checkout_tabs_wp_ml_splash_enabled" value="1" ' . ($splash_enabled ? 'checked' : '') . ' /> Exibir no primeiro carregamento do site</label></td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th scope="row">Fundo</th>';
		echo '<td><input type="color" name="checkout_tabs_wp_ml_splash_bg" value="' . esc_attr($splash_bg) . '" /></td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th scope="row"><label for="checkout_tabs_wp_ml_splash_image_url">URL da imagem (logo/GIF)</label></th>';
		echo '<td><input type="url" class="regular-text" id="checkout_tabs_wp_ml_splash_image_url" name="checkout_tabs_wp_ml_splash_image_url" value="' . esc_attr($splash_image_url) . '" placeholder="https://..." /></td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th scope="row"><label for="checkout_tabs_wp_ml_splash_duration_ms">Tempo (ms)</label></th>';
		echo '<td><input type="number" id="checkout_tabs_wp_ml_splash_duration_ms" name="checkout_tabs_wp_ml_splash_duration_ms" value="' . esc_attr((string) $splash_duration_ms) . '" min="0" step="50" style="width:120px;" /> <span class="description">Corte seco (sem transição).</span></td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th scope="row">Texto abaixo da imagem</th>';
		echo '<td>';
		echo '<label><input type="checkbox" name="checkout_tabs_wp_ml_splash_text_enabled" value="1" ' . ($splash_text_enabled ? 'checked' : '') . ' /> Exibir texto</label>';
		echo '<div style="margin-top:10px; display:flex; gap: 16px; flex-wrap: wrap; align-items: center;">';
		echo '<label style="display:flex; gap:8px; align-items:center;">Cor <input type="color" name="checkout_tabs_wp_ml_splash_text_color" value="' . esc_attr($splash_text_color) . '" /></label>';
		echo '<label style="display:flex; gap:8px; align-items:center;">Fonte <input type="text" name="checkout_tabs_wp_ml_splash_text_font" value="' . esc_attr($splash_text_font) . '" class="regular-text" style="width:180px;" /></label>';
		echo '<label style="display:flex; gap:8px; align-items:center;">Gap (px) <input type="number" name="checkout_tabs_wp_ml_splash_text_gap_px" value="' . esc_attr((string) $splash_text_gap_px) . '" min="10" step="1" style="width:80px;" /></label>';
		echo '<label style="display:flex; gap:8px; align-items:center;"><input type="checkbox" name="checkout_tabs_wp_ml_splash_text_typing" value="1" ' . ($splash_text_typing ? 'checked' : '') . ' /> Animação (digitação)</label>';
		echo '</div>';
		echo '<div style="margin-top:10px;">';
		echo '<input type="text" class="regular-text" name="checkout_tabs_wp_ml_splash_text" value="' . esc_attr($splash_text) . '" placeholder="Texto da splash..." />';
		echo '</div>';
		echo '</td>';
		echo '</tr>';
		echo '</table>';
